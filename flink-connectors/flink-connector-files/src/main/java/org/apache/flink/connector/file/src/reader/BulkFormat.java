 ...
 * <h2>Checkpointing</h2>
 *
 * <p>The bulk reader returns an iterator per batch that it reads. The iterator produces records
 ...
 * etc.
 *
 * <p>Formats that have the ability to efficiently seek to a record (or to every n-th record) can
 ...
 *
 * <p>For the {@code BulkFormat}, one batch (as returned by {@link BulkFormat.Reader#readBatch()})
...
public interface BulkFormat<T, SplitT extends CheckpointableSourceSplit>
    ...
    interface Reader<T> extends Closeable {
         ...
         *
         * <p>The returned iterator object and any contained objects may be held onto by the file
